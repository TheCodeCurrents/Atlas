...
impl<'a> Parser<'a> {
    pub fn new(src: &'a str) -> Self {
        Self {
            lexer: Lexer::new(src),
            pos: 0,
            symbols: Self::build_symbol_table(src),
            last_line: 1,
        }
    }

    fn build_symbol_table(src: &'a str) -> SymbolTable {
        let mut lexer = Lexer::new(src);
        let mut table = SymbolTable::new();
        let mut pos: u32 = 0;

        while let Some(token_result) = lexer.next() {
            let spanned = match token_result {
                Ok(token) => token,
                Err(_) => break,
            };
            ...
            match spanned.token {
                ...
                Token::Directive(Directive::Import) => {
                    ...
                    if let Some(Ok(next_tok)) = lexer.next() {
                        if let Token::LabelRef(name) = next_tok.token {
                            table.insert(name, crate::parser::symbols::Symbol::External);
                        }
                    }
                }
                Token::Mnemonic(_) => {
